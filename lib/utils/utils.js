...
const config = require("../../bin/constant/setup");
// const { staticResponseMessageObject } = require('../../lib/responseMessages/message');
// const { awsContentImageUrl } = require('../../constant/setup')
    ...
    if (list && list.length) {
      result = await Promise.all(
        list.map((oneRecord) => {
          ...
          oneRecord.image = (
            path.join(__dirname, "/../../", "/public/upload/") + oneRecord.image
          ).replace(/\\/g, "/");
          return oneRecord;
        })
      );
...
async function getOneImageUrL(image) {
  try {
    ...
    image = (
      path.join(__dirname, "/../../", "/public/upload/") + image
    ).replace(/\\/g, "/");
    return image;
  } catch (error) {
    throw new Error(error.message);
...
module.exports = {
  ...
  getFilter: async (options) => {
    ...
    if (options.page && options.limit) {
      if (options.limit > process.env.recordLimit) {
        filter.skip = (options.page - 1) * process.env.recordLimit;
        filter.limit = process.env.recordLimit;
      } else {
  ...
  //       let docContentType = await isBase64(base64Document, {
  //         allowMime: true,
  //       });
  //       if (!docContentType) {
  //         return reject(new Error("Image must be in base64 format"));
  //       }
  //       const base64 = base64Document.indexOf(";base64,");

  //       const docExtension = base64Document.substring(
  //         "data:image/".length,
  //         base64Document.indexOf(";base64")
  //       );
  //       docContentType = base64Document.substring("data:".length, base64);
  //       const buffer = Buffer.from(
  //         base64Document.replace(/^data:image\/\w+;base64,/, ""),
  //         "base64"
  //       );
  //       const regex = / /gi;
  //       const fileName =
  //         documentName.replace(regex, "-") +
  //         "-" +
  //         new Date().getTime() +
  //         "." +
  //         docExtension;
  ...
  // getOneImageUrL,
  // getThumbnail: (item) => {
  //   return awsContentImageUrl + item.worky_id + "_1.jpg";
  // }
};
